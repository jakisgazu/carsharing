...
class CarsharingSupabaseUser extends BaseAuthUser {
  CarsharingSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> carsharingSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = CarsharingSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
